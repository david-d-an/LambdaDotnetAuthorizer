...
```bash
LambdaStandAlone$ sam local invoke helloFromLambdaFunction --no-event
```
...
Resources:
  ...
  helloFromLambdaFunction:
    ...
    Properties:
      CodeUri: ./src/LambdaStandAlone/
      Handler: LambdaStandAlone::LambdaStandAlone.AuthorizerFunction::FunctionHandler
...
```bash
LambdaStandAlone$ sam logs -n helloFromLambdaFunction --stack-name sam-app --tail
```
...
```bash
LambdaStandAlone$ dotnet test test/LambdaStandAlone.Tests/LambdaStandAlone.Tests.csproj 
```
...
```bash
aws cloudformation delete-stack --stack-name LambdaStandAlone
```
